  ...
  return (
    <main>
      ...
      <section className="relative overflow-hidden pt-24">
        {/* background image does not intercept clicks */}
        ...
        <div className="relative container mx-auto px-6 py-20">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <a
              className="inline-flex items-center rounded-2xl bg-white text-black px-5 py-3 font-semibold"
              href={`tel:${PHONE}`}
            >
              {t.callNow}
            </a>
            <a
              className="inline-flex items-center rounded-2xl border border-white/30 px-5 py-3"
              href={`sms:${SMS}`}
            >
              {t.sms}
            </a>
          ...
          {/* Quick quote (UI only) */}
          <form
            className="mt-10 grid md:grid-cols-4 gap-3 bg-white/10 border border-white/10 rounded-2xl p-4 backdrop-blur"
            role="search"
          ...
          >
            ...
            <input
              ...
              className="rounded-xl bg-black/30 border border-white/10 px-4 py-3"
            />
            {/* prevent form submit/reload since this is UI-only in a server component */}
            <button
              type="button"
              className="rounded-2xl bg-white text-black px-6 font-semibold flex items-center justify-center"
            >
              {t.search}
            </button>
          </form>
      ...
      <section className="container mx-auto px-6 pb-20 grid md:grid-cols-2 gap-8 items-center">
        <div className="relative h-72 md:h-96 rounded-2xl overflow-hidden border border-white/10">
          <Image
            src="/img/image02.png"
            alt="Driver helping with luggage"
            fill
        ...
        <div>
          ...
          <div className="mt-4 flex gap-3">
            <Link
              ...
              href={`/${params.locale}/services/airport-transfers`}
            >
              {a.airportTransfers}
            </Link>
            <Link
              className="rounded-2xl border border-white/20 px-4 py-2"
              href={`/${params.locale}/services/hourly-hire`}
            >
              {a.hourlyHire}
            </Link>
          </div>
